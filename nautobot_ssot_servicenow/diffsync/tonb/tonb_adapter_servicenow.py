...
class ServiceNowDiffSync(DiffSync):
    ...
    def load_table(self, modelname, table, mappings, **kwargs):
        ...
        # self.joblog_debug(f"Loading table {table} into {modelname} instances...")
        ...
        if "parent" not in kwargs:
            ...
            for parent in self.get_all(kwargs["parent"]["modelname"]):
                # self.joblog_debug(f"Loading children of {parent}")
                print(f"{parent} - {parent.sys_id}")
                print(f"{kwargs}")
    ...
    def load_record(self, table, record, model_cls, mappings, **kwargs):
        ...
        try:
            ...
            # self.joblog_debug(f"Duplicate object encountered for {modelname} {model.get_unique_id()}")
            ...
            if "column" in mapping:
                ...
                if "key" in mapping["reference"]:
                    ...
                    if key not in record:
                        pass
                        # self.joblog_debug(f"Key {key} is not present in record {record}")
                    else:
                        sys_id = record[key]
                else:
